...
export default function MapView({ schools, mode, onOpenSchool, onSelect, selectedId }) {
  ...
  useEffect(() => {
    ...
    instance.on('load', () => paint(instance, schools));
  ...
  useEffect(() => {
    const instance = map.current;
    if (!instance) return;
    const nextStyle = STYLES[mode] || STYLES.heat;
    ...
    const current = instance.getStyle()?.sprite?.includes('satellite') ? 'satellite' : 'light';
    const wantSatellite = mode === 'satellite';
    if ((current === 'satellite') !== wantSatellite) {
      instance.once('styledata', () => { paint(instance, schools); applyVisibility(); });
      instance.setStyle(nextStyle);
    } else if (instance.isStyleLoaded()) {
      applyVisibility();
    } else {
      instance.once('load', applyVisibility);
    }
  }, [mode, schools, paint]);
  ...
  const reset = () => map.current?.flyTo({ center: [82.9, 49.4], zoom: 6.1, speed: 1.4 });

  return (
    <>
      <div ref={holder} className="map-root" />
